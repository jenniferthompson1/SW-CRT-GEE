...
clusterSetRNGStream(cl=cl, 64210)
...
v.rhoc <-  c(0.01, 0.1)
v.clusters <-  c(18)
v.clustersize <-  c(24, 60)
v.cv <- c(0, 0.4)

# Functions ---------------------------------
## Correction functions =================================

source("./DataGeneration/functions.R")
...
createdata <- function(x,cps) {
  ...
  # Check eigne values are positive for correlation matrix. Random variation and rounding mean some of the big matrices have megative eigen values
...
replicatedata <- function(A) {
  ...
  if (is(data,"warning") | is(data,"error")) return((list(list(iscenario, .Random.seed), NA)))
  ...
  for (wc in c("independence","exchangeable")) {
    ...
    if(is(geemdl,"warning") | is(geemdl,"error") )  {
      ...
      results <- rbind(results,
                       c(iscenario, iwc, sampsi, rep(NA,17)))
      ...
      for (sa in c("lz", "fg", "mk", "kc", "md", "mbn")) {
        ...
        if (is(var,"warning") | is(var,"error")) {
          ...
          row <- c(row,
                   sqrt(var$cov.beta[2]),
                   2 * var$cov.beta[2]^2 / var$cov.var[2,2])
      ...
      if(is(saws,"warning") | is(geemdl,"error") )  sdf <- NA else sdf <- saws$df[2]
      
      #Simple:
      ## Clusters minus parameters
      ## Cluster*periods minus parameters
      ## clusters*periods minus clusters minus parameters
      
      row <- c(row,
               sdf,
               clusters - (sequences + 1),
               clusters * sequences - (sequences + 1),
               clusters * sequences - (clusters + sequences))
          ...
          for (clustersize in v.clustersize) {
            ...
            for (cv in v.cv) {
              ...
              clusterExport(cl,c("cv","clustersize","cps","clusters","sequences", "corrscenario", "rhoc",
                                 ...
                                 "GEEGLM.var.lz", "GEEGLM.var.fg", "GEEGLM.var.kc", "GEEGLM.var.md", "GEEGLM.var.mbn", "GEEGLM.var.mk", 
...
save(all.results, file = "./DataGeneration/geeresults_phase1.RData")

colnames(key.scenario) <- c("scenario", "sequences", "clusters", "mean cluster size", "cv cluster size", "icc", "corrscenario")

save(key.scenario, file = "./DataGeneration/key.scenario_phase1.RData")


save(all.randomseeds, file = "./DataGeneration/allrandomseeds_phase1.RData")
proc.time() - ptm
