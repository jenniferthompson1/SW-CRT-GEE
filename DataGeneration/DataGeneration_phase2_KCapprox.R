...
createdata <- function(x,cps) {
  ...
  # Check eigne values are positive for correlation matrix. Random variation and rounding mean some of the big matrices have negative eigen values
  ...
  for (wc in c("independence","exchangeable")) {
    ...
    if(is(geemdl,"warning") | is(geemdl,"error") )  {
      ...
      results <- rbind(results,
                       c(iscenario, iwc, sampsi, rep(NA, 3)))
...
save(all.results, file = "./DataGeneration/geeresults_phase2_kcapprox.RData")

colnames(key.scenario) <- c("scenario", "sequences", "clusters", "mean cluster size", "cv cluster size", "icc", "corrscenario")

save(key.scenario, file = "./DataGeneration/key.scenario_phase2_kcapprox.RData")


save(all.randomseeds, file = "./DataGeneration/allrandomseeds_phase2_kcapprox.RData")
proc.time() - ptm
