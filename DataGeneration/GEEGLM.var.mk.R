...
function(formula,id,family=gaussian,data,corstr="independence"){
  ...
  for (i in 1:size){
    ...
    var_i=var[1:cluster$n[i],1:cluster$n[i]]
    if (family=="gaussian"){
       xy<-t(covariate)%*%solve(var_i)%*%(y-covariate%*%beta_est)
  ...
  cov.var<-scale^2*size/(size-1)*kronecker(solve(step11), solve(step11))%*%step14%*%kronecker(solve(step11), solve(step11)) 
