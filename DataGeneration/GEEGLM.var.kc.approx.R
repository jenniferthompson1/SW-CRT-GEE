...
function(formula,id,family=gaussian,data,corstr="independence"){
  ...
  gee.fit <- geeglm(formula,data=data,id=id,family=family,corstr=corstr)
  beta_est <- gee.fit$coefficient
  alpha <- gee.fit$geese$alpha
  len <- length(beta_est)
  len_vec <- len^2
  
  ### Estimate the robust variance for \hat{\beta}
  data$id <- gee.fit$geese$id
  ...
  if(is.character(corstr)){
    ...
    if (corstr == "unstructured") {
      unstrcor <- cormax.ind(ncluster)
      unstrcor[lower.tri(unstrcor)] <- alpha
      unstrcor <- t(unstrcor)
      unstrcor[lower.tri(unstrcor)] <- alpha
    }
    var <- switch(corstr,
  ...
  for (i in 1:size){
    ...
    if (family=="gaussian"){ 
      ...
      # JT 25 june 2020: replacing true KC with approximation suggested by gallis 2020 and ford 2018
        ...
        # xy<-t(covariate)%*%
        #   solve(var_i)%*%
        #   mat.sqrt.inv(cormax.ind(cluster$n[i])-covariate%*%solve(step11)%*%t(covariate)%*%solve(var_i))%*%
      ...
      yx <- t(covariate)%*%
       ...
       sumyx <- sumyx + yx
     	...
     	Vi <- diag(sqrt(c(exp(covariate%*%beta_est))),cluster$n[i])%*%
     	  var_i%*%
     	  diag(sqrt(c(exp(covariate%*%beta_est))),cluster$n[i])
       
     # 	 xy<-t(D)%*%
     # 	   solve(Vi)%*%
     # 	   mat.sqrt.inv(cormax.ind(cluster$n[i])-D%*%solve(step11)%*%t(D)%*%solve(Vi))%*%
     ...
     #    step12<-step12+xy%*%t(xy)
       ...
       Vi <- diag(sqrt(c(exp(covariate%*%beta_est)/(1+exp(covariate%*%beta_est))^2)),cluster$n[i])%*%
         var_i%*%
         diag(sqrt(c(exp(covariate%*%beta_est)/(1+exp(covariate%*%beta_est))^2)),cluster$n[i])
